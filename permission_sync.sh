...
log_action() {
    ...
    local new_perm="$4"
    local timestamp=$(date '+%Y-%m-%d %H:%M:%S')
    echo "[$timestamp] ACTION: $action | FILE: $file | OLD: $old_perm | NEW: $new_perm" | tee -a "$TEMP_LOG"
}
...
compare_and_fix() {
    ...
    if [[ "$fix_mode" = true ]]; then
        echo ""
        read -p "Create backup before making changes? (yes/no) [yes]: " create_backup
